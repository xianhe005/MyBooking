...
import com.cdrcb.timepickerlibrary.R;
...
public class TimeWheel {
    ...
    void updateMonths() {
        ...
        //////////////////////修改下面BUG start//////////////////////
        int delta = -1;
        NumericWheelAdapter numericWheelAdapter = (NumericWheelAdapter) month.getViewAdapter();
        if (numericWheelAdapter != null) {
            int oldMinValue = numericWheelAdapter.getMinValue();
            int oldMonth = oldMinValue + month.getCurrentItem();

            if (oldMonth < minMonth) {
                delta = 0;
            } else if (oldMonth > maxMonth) {
                delta = maxMonth - minMonth;
            } else {
                month.setCurrentItem(oldMonth - minMonth);
            }
        }
        //////////////////////修改下面BUG end//////////////////////
        month.setViewAdapter(mMonthAdapter);

        /*if (mRepository.inYear(curYear)) {
            month.setCurrentItem(0, false);
        }*/
        // TODO: 2018/6/8 0008 上面原代码有BUG，暂时这样避免出现空白情况
        if (delta >= 0) {
            month.setCurrentItem(delta, true);
        }
    }
